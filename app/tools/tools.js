...
async function researchAgent(item) {
  ...
  let researchItem = researchItems.find((researchItem) => researchItem.name === item.type);
  
  // If this is a generic research task with web-related keywords, mark it as web research
  if (researchItem && !researchItem.webResearch) {
    const webKeywords = ['github', 'repository', 'open source', 'popular', 'trending'];
    const queryLower = (item.query || '').toLowerCase();
    if (webKeywords.some(keyword => queryLower.includes(keyword))) {
      researchItem = { ...researchItem, webResearch: true };
    }
  }
